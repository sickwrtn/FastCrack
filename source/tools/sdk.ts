...
class my_struct implements interfaces.my_struct{
 ...
 class message_struct implements interfaces.message_struct{
    ...
    set(content: string): interfaces.response{
        let request = requests.patchAfetch(env.wrtn_api2 + `/character-chat/characters/chat/${this.json.chatId}/message/${this.json._id}`,{
            message:content
        });
        return request;
    }
  ...
  class chatroom_struct implements interfaces.chatroom_struct{
    ...
    async getMessages(cursor: string="",load_limit: number): Promise<interfaces.response | null>{
        if (cursor != "") {
            var request = await fetch(env.wrtn_api2 + `/character-chat/api/v2/chat-room/${this.json._id}/messages?limit=${load_limit}&cursor=${cursor}`, this.getHeader);
            debug(`GET ${env.wrtn_api2 + `/character-chat/api/v2/chat-room/${this.json._id}/messages?limit=${load_limit}&cursor=${cursor}`}`,2);
        }
        if (cursor == null){
            return null;
        }
        else {
            var request = await fetch(env.wrtn_api2 + `/character-chat/api/v2/chat-room/${this.json._id}/messages?limit=${load_limit}`, this.getHeader);
            debug(`GET ${env.wrtn_api + `/character-chat/api/v2/chat-room/${this.json._id}/messages?limit=${load_limit}`}`,2);
        }
    ...
    send(content: string,IsSuperMode: boolean): interfaces.message_struct{
        let created_msg = requests.postAfetch(env.wrtn_api2 + `/character-chat/characters/chat/${this.json._id}/message`, {
            message: content,
            reroll: false,
            images: [],
  ...
  export class wrtn_api_class implements interfaces.wrtn_api_class{
    ...
    getRepresentativePersona(): interfaces.characterChatProfile | null{
        let wrtn_uid = requests.getAfetch(env.wrtn_api + '/character/character-profiles').data._id;
        let character_profiles = requests.getAfetch(env.wrtn_api + `/character/character-profiles/${wrtn_uid}/character-chat-profiles`).data.characterChatProfiles;
        //대표 프로필 가져오기
    ...
    async getMycharacters(cursor: string,load_limit: number): Promise<interfaces.response | null> {
        if (cursor != ""){
            var request = await fetch(env.wrtn_api + `/character/characters/me?limit=${load_limit}&cursor=${cursor}`, this.getHeader);
            debug("GET " + env.wrtn_api + `/character/characters/me?limit=${load_limit}&cursor=${cursor}`,2);
        }
        else if (cursor == null){
            return null;
        }
        else{
            var request = await fetch(env.wrtn_api + `/character/characters/me?limit=${load_limit}`, this.getHeader);
            debug("GET " + env.wrtn_api + `/character/characterss/me?limit=${load_limit}`,2);
        }
    ...
    async character_search(query: string,cursor: string,sort: string,load_limit: number): Promise<interfaces.response | null>{
        if (cursor != ""){
            var request = await fetch(env.wrtn_api + `/character/characters/search?limit=${load_limit}&query=${query}&sort=${sort}&cursor=${cursor}`);
            debug("GET " + env.wrtn_api + `/character/characters/search?limit=${load_limit}&query=${query}&sort=${sort}&cursor=${cursor}`,2);
        }
        else if (cursor == null){
            return null;
        }
        else{
            var request = await fetch(env.wrtn_api + `/character/characters/search?limit=${load_limit}&query=${query}&sort=${sort}`);
            debug("GET " + env.wrtn_api + `/character/characters/search?limit=${load_limit}&query=${query}&sort=${sort}`,2);
        }
    ...
    getChatroom(chatId: string): interfaces.chatroom_struct{
        let request = requests.getAfetch(env.wrtn_api2 + `/character-chat/api/v2/chat-room/${chatId}`);
